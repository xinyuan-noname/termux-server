...
class WebSocketController {
    ...
    */
    ...
    /**
     * 向指定用户列表推送自定义消息, 离线用户的消息进入待发送队列
     * @param {Array<string>} idList - 目标用户 ID 列表
     * @param {Object} message - 消息对象
     * @returns {{onlineCount:number,offlineCount:number}} 推送统计
     */
    static pushMessage(idList, message) {
        const messageStr = JSON.stringify(message);
        const { clientList, offlineIdList } = WebSocketController.getClientFromIdList(WebSocketController.TaskClientMap, idList);
        let onlineCount = 0;
        for (const client of clientList) {
            for (const socket of client.wsList) {
                try {
                    socket.send(messageStr);
                    onlineCount++;
                } catch (err) {
                    logger.warn(`${client.id}推送消息失败`, err);
                }
            }
        }
        for (const userId of offlineIdList) {
            MessageServer.cacheRemind({ data: messageStr, userId, sent: false })
                .catch((err) => logger.warn(`${userId}缓存离线消息失败`, err));
        }
        return { onlineCount, offlineCount: offlineIdList.length };
    }
